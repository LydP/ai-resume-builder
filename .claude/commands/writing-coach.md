...
| Utilized | Used / Applied / Deployed |
| In order to | To |
...
### Rule 9: Opening Role as the Hook

There is no Professional Summary in this format. Work Experience leads the resume. The first role's company overview bullet (Rule 15) IS the opening impression — write it as the most compelling thing the reader will see in the first 10 seconds. Lead with the company's biggest number, its market position, or its domain significance before anything else.
...
### Rule 11: Plain Language
The primary audience for every bullet is a human being — a recruiter or hiring manager scanning quickly, not parsing technical documentation. Write for that person first. Every bullet should use the simplest phrasing that accurately conveys the meaning. When a technical or abstract phrase can be replaced by plain English without losing precision, replace it.

- **Test:** Read the phrase aloud. Is there a simpler way to say the same thing? If yes, use it.
- **Technical language exception:** Retain technical terms when they appear in or are clearly implied by the JD — they serve ATS keyword matching and signal relevant skills. Outside of that, plain English always wins.
- **Action verb exception:** Power verbs (Rule 5) are exempt from the plain language preference. Words like "Directed", "Architected", and "Pioneered" are industry-standard resume vocabulary — recruiters read them fluently, they signal seniority, and simpler alternatives ("ran", "built", "started") often understate the scope. Apply the plain language test to sentence structure and descriptive phrases, not to action verb choice.

```
OVERLY COMPLEX: Applied UMAP dimensionality reduction and clustering to segment publisher quality tiers
PLAIN:          Applied UMAP clustering to determine publisher quality
```

### Rule 12: Master Resume Source Material

The master resume uses a three-part format for named projects. Read each part differently:

- **Paragraph** — primary synthesis source. Understand the project from this. Generate tailored bullets by synthesizing the paragraph into 1-3 output bullets that best match the JD.
- **Tools line** — keyword metadata. Scan when matching against JD keywords; tools listed here are available to surface in bullets even if not mentioned in the paragraph.
- **Details sub-list** — reference material. Consult when the JD emphasizes something specific (a method, stakeholder process, validation approach) that isn't fully captured in the paragraph. Never enumerate the Details sub-list directly into output bullets.

**The synthesis rule:** A named project produces at most 3 output bullets, regardless of how many detail items exist. The paragraph drives what those bullets say; the Details sub-list informs specific wording when relevant.

---

## SECTION-SPECIFIC WRITING GUIDELINES

### Work Experience Lead Bullet (Rule 15)
- First bullet of each role = company context (revenue, users, Fortune 500 rank, headcount, funding, etc.)
- If company stats are unavailable from master resume, use domain/product/market position
- Do not fabricate — only use context that can be verified or inferred from master resume

### Key Results Sub-bullets (Rule 16)
- Nest under the relevant parent bullet, indented with `-`
- Use for: revenue, cost savings, man-hours saved, users/partners acquired, accuracy %, tech stack
- Each sub-bullet: one concise line (10-20 words)
...
- Include GPA only if 3.3+
- Relevant coursework only if entry-level or career-change
- Keep formatting minimal and scannable

### Certifications, Skills & Interests
- All non-experience sections combined into one block
- Use labeled bullets: Certifications, Technologies, Skills, Publications, Memberships, Interests
- Omit any bullet whose category is empty
- Interests: 4-6 items; makes the candidate feel human to the hiring team
...
1. **Before reading source material**: Apply Rule 12 (Master Resume Source Material) — identify paragraphs, Tools lines, and Details sub-lists; treat each correctly before writing anything
2. **Before writing any bullet**: Mentally run through Rules 1-11
3. **First role's lead bullet**: Apply Rule 15 (Company Overview) — this is now the opening impression
4. **Each bullet point**:
   ...
   - Apply Rule 11 (Plain Language)
5. **Per role block**: Apply Rule 7 (Rhythm) and Rule 8 (Parallel Structure)
6. **Final pass**: Apply Rule 10 (Authenticity Anchor / Interview Test)
